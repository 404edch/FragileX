...
export default function PreencherChecklist({ isRapido = false }: Props) {
  ...
  useEffect(() => {
    ...
    if (!isRapido && usuario && usuario.role === 'paciente') {
      ...
      api.get(`/patients/cpf/${usuario.cpf}`)
        .then(res => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const quemPreencheFinal = quemPreencheSelecionado === 'Outro' 
      ? (quemPreencheOutro.trim() ? `Outro: ${quemPreencheOutro.trim()}` : '') 
    ...
    if (isRapido) {
      ...
      const calculos = sintomasSelecionados.map(id => {
        const s = sintomasList.find(item => item.id === id);
  ...
  const renderReportModal = () => {
    ...
    return (
      <div style={{
        ...
        <div className="dashboard-med-registration" style={{ maxWidth: '600px', width: '100%', background: '#fff', borderRadius: '16px', padding: '32px', maxHeight: '90vh', overflowY: 'auto' }}>
  ...
  return (
    <div className="checklist-wrapper">
      ...
      {step === 1 && (
        <div className="cadastro-form" style={{ maxWidth: '500px', margin: '0 auto' }}>
          <div className="checklist-container">
            ...
            {isRapido ? (
              <form onSubmit={handleStartRapido}>
                ...
                <div style={{ display: 'flex', gap: '16px', marginBottom: '24px' }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
                    <input type="radio" name="sexo" value="M" onChange={() => setSexoRapido('M')} required /> Masculino
                  </label>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
                    <input type="radio" name="sexo" value="F" onChange={() => setSexoRapido('F')} required /> Feminino
                  </label>
                </div>
                <button type="submit" className="checklist-submit-btn" style={{ width: '100%' }}>Avançar para Sintomas</button>
              </form>
              ...
              <form onSubmit={handleBuscarPaciente}>
                ...
                <div className="cadastro-item" style={{ marginBottom: '24px' }}>
                ...
                <button type="submit" className="checklist-submit-btn" style={{ width: '100%' }} disabled={isLoadingPatient}>
        ...
        <form className="cadastro-form" onSubmit={handleSubmit}>
          {!isRapido && patientDetails && (
            <div className="checklist-container" style={{ marginBottom: "24px" }}>
              <h2 className="checklist-title" style={{ fontSize: '1.5rem' }}>Ficha do Paciente</h2>
              <div style={{ background: '#f8fafc', padding: '16px', borderRadius: '8px', border: '1px solid #e2e8f0', fontSize: '14px', marginBottom: '16px' }}>
                <p><strong>Nome:</strong> {patientDetails.nome}</p>
                <p><strong>CPF:</strong> {patientDetails.cpf}</p>
                <p><strong>Sexo Biológico:</strong> {patientDetails.pacienteDetails?.sexo_biologico === 'M' ? 'Masculino' : 'Feminino'}</p>
              </div>

              {usuario?.role === 'paciente' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  <div className="cadastro-item" style={{ marginBottom: 0 }}>
          ...
          <ErrorBoundary
            fallback={
              <div className="checklist-container checklist-error-container">
                ...
                <button className="checklist-retry-btn" onClick={() => window.location.reload()}>Tentar Novamente</button>
              </div>
            }
          >
            <Suspense fallback={<div className="checklist-container"><h2 className="checklist-loading">Carregando checklist...</h2></div>}>
              <div className="checklist-container">
                <h1 className="checklist-title">
                  {isRapido ? "Checklist Rápido" : "Checklist Clínico (X Frágil)"}
                </h1>
