    ...
    <div style={{ padding: '24px', maxWidth: '1000px', margin: '0 auto' }}>
      <div style={{ marginBottom: '24px' }}>
        ...
        <p style={{ color: '#4a5568', fontSize: '14px' }}>
          Lista de todos os médicos credenciados na plataforma.
        </p>
      </div>
        ...
        <AnimatedList>
          {doctors.map(doc => (
            <div key={doc.id} style={{
              ...
              <div>
                <h3 style={{ margin: '0 0 8px 0', fontSize: '18px', color: '#0f172a', fontWeight: 'bold' }}>
                  {doc.nome}
                </h3>
                <div style={{ display: 'flex', gap: '20px', fontSize: '14px', color: '#64748b' }}>
                  <span><strong>E-mail:</strong> {doc.email}</span>
                  <span><strong>CRM:</strong> {doc.medicoDetails?.crm || 'Não informado'}</span>
                  <span><strong>Telefone:</strong> {doc.telefone || 'Não informado'}</span>
                </div>
              </div>
              <div style={{ textAlign: 'right' }}>
                ...
                <p style={{ margin: '8px 0 0 0', fontSize: '13px', color: '#64748b' }}>
                  {doc.medicoDetails?.especialidade || 'Clínico Geral'}
                </p>
              </div>
            </div>
          ))}
